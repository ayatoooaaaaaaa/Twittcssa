...
# ⚠️ URLのズレを防ぐため、あなたのサイトのURLを完全に固定します！
...
st.write("URLの中にある `code=` から後ろの英数字をコピーして、すぐに下の欄に貼り付けてください。")

user_code = st.text_input("ここにコピーしたコードを入力", key="user_code_input")
        ...
        try:
            with st.spinner("トークンを回収中..."):
                ...
                if "access_token" not in res_data:
                    st.error(f"X側でエラーが発生しました。もう一度リンクを踏み直してください: {res_data}")
                else:
    ...
    if st.button("🚀 ツイートを取得して削除を開始する", type="primary", key="del_btn"):
        ...
        with st.spinner("削除中..."):
            try:
                ...
                st.error(f"ツイートの取得・削除中にエラーが発生しました: {e}")
