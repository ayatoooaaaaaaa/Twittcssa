...
st.write("Xの画面で『アプリにアクセスを許可』を押した後、アドレスバーのURLを確認してください。")
st.write("URLの中にある `code=` から後ろの英数字を、**コピーしてすぐに**下の欄に貼り付けてください！")

user_code = st.text_input("ここにコピーしたコードを入力（時間との勝負です！）", key="user_code_input")

if user_code:
    # ユーザーがコードを入力した瞬間、ボタンを待たずに即時実行する安全弁も用意
    if st.button("🔑 このコードでログインして削除画面を開く", type="primary"):
        try:
            with st.spinner("賞味期限内にトークンを回収中..."):
                ...
                if "access_token" not in res_data:
                    st.error(f"X側でコードの期限が切れたか、エラーが発生しました。もう一度リンクを踏み直してください: {res_data}")
                else:
...
if "x_client" in st.session_state:
    ...
    if st.button("🚀 ツイートを取得して削除を開始する", type="primary", key="del_btn"):
        ...
        with st.spinner("削除中..."):
            try:
                ...
                st.error(f"ツイートの取得・削除中にエラーが発生しました（APIの回数制限の可能性があります）: {e}")
